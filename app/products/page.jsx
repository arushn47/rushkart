...
import { useSearchParams } from 'next/navigation'; // Import for reading URL parameters
...
const FilterBar = ({ categories, activeCategory, onSelectCategory }) => (
    <div className="flex flex-wrap items-center gap-2 md:gap-4 my-8">
        <button
            ...
            className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activeCategory === 'All' ? 'bg-indigo-600 text-white' : 'bg-white/50 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
        >
            All
        </button>
        {categories.map(category => (
            <button
                ...
                className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activeCategory === category ? 'bg-indigo-600 text-white' : 'bg-white/50 dark:bg-slate-800/50 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
            >
                {category}
            </button>
        ))}
